...
export-env {
    ...
    let virtual_env = r#'C:\Users\alexa\PycharmProjects\project1_minyushkina_zoya_M25-555\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'project1-minyushkina-zoya-m25-555-py3.12'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'project1-minyushkina-zoya-m25-555-py3.12'#
    }
